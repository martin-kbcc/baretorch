...
# Hardware Setup (Dual RTX 4090 Workstation)
NUM_GPUS=2

# 2.5B Model Architecture (D_MODEL=2048, LAYERS=36, VOCAB=248k)
...
# Optimization (Local Test Run)
PER_GPU_BATCH_SIZE=2    # Low batch size to prevent VRAM OOM on 24GB GPUs
GRAD_ACCUM=16           # Effective batch size = 64 seqs/step (~131k tokens/step)
LEARNING_RATE=3e-4
SCHEDULER="wsd"
WARMUP_STEPS=100
WEIGHT_DECAY=0.1
...
# Paths
OUTPUT_DIR="${ROOT_DIR}/checkpoints_distill_2.5B_local"
DATA_CACHE_DIR="${ROOT_DIR}/teacher_predictions_sample"
...
echo "======================================================================"
echo "🚀 Launching BareTorch Local Distillation Test (2.5B Hybrid)..."
