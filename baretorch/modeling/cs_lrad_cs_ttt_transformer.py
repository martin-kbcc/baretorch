...
# Direct, dry imports from your modular sequence blocks to maintain clean architecture
...
from baretorch.modeling.cs_ttt import TTTDecoderBlock


# ==========================================
# 1. Hugging Face Triple-Hybrid Configuration
# ==========================================

class CSLRADCSTTTTransformerConfig(PretrainedConfig):
    model_type = "cs_lrad_cs_ttt_transformer"
    ...
    def __init__(
        ...
        num_heads=16,
        num_kv_heads=4,
        num_layers=12,  # Optimized default layer depth
        chunk_size=32,
        rank=8,
        dropout=0.1,
        max_seq_len=4096,
        use_grad_checkpointing=False,
        layer_types=None,  # Optional explicit sequence mixing topology layout
        pad_token_id=0,
        bos_token_id=1,
        eos_token_id=2,
        **kwargs,
        ...
        # Default rotation: Alternating Transformer -> CS-LRAD -> CS-TTT
...
# ==========================================
# 2. Triple-Hybrid Model Implementations
# ==========================================

class CSLRADCSTTTTransformerPreTrainedModel(PreTrainedModel):
    config_class = CSLRADCSTTTTransformerConfig
...
class CSLRADCSTTTTransformerModel(CSLRADCSTTTTransformerPreTrainedModel):
    def __init__(self, config):
        ...
        # Build the heterogeneous layers sequentially
    ...
    def forward(
        ...
        # Apply padding dynamically for the sub-quadratic parallel prefill paths (CS-LRAD and CS-TTT)
        pad_len = 0
...
class CSLRADCSTTTTransformerForCausalLM(CSLRADCSTTTTransformerPreTrainedModel):
    _tied_weights_keys = {"lm_head.weight": "model.token_embedding.weight"}

    def __init__(self, config):
        super().__init__(config)
        self.model = CSLRADCSTTTTransformerModel(config)
    ...
    def _reorder_cache(self, past_key_values, beam_idx):
        ...
        for i, layer_past in enumerate(past_key_values):
            ...
            else:  # Is a sub-quadratic state tensor (S_state from CS-LRAD or S from CS-TTT)
