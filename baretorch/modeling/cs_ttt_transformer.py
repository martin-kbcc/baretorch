...
class CSTTTTransformerConfig(PretrainedConfig):
    model_type = "cs_ttt_transformer"
    ...
    def __init__(
        ...
        layer_types=None,  # Optional list of strings specifying layer order, e.g. ["transformer", "cs_ttt", ...]
        pad_token_id=0,
        bos_token_id=1,
        eos_token_id=2,
        **kwargs,
...
class CSTTTTransformerPreTrainedModel(PreTrainedModel):
    config_class = CSTTTTransformerConfig
...
class CSTTTTransformerModel(CSTTTTransformerPreTrainedModel):
    ...
    def forward(
        ...
        # Apply padding dynamically for the CS-TTT parallel prefill paths
        pad_len = 0
...
class CSTTTTransformerForCausalLM(CSTTTTransformerPreTrainedModel):
    _tied_weights_keys = {"lm_head.weight": "model.token_embedding.weight"}

    def __init__(self, config):
        super().__init__(config)
        self.model = CSTTTTransformerModel(config)
    ...
    def _reorder_cache(self, past_key_values, beam_idx):
        ...
        for i, layer_past in enumerate(past_key_values):
            ...
            else:  # Is a CS-TTT state tensor (S)
