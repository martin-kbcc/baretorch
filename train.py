...
class MemmapDataset(Dataset):
  """High-Performance, zero-copy PyTorch Dataset mapping directly over flat

  uint16 pre-tokenized binary files using numpy.memmap.
  """
  ...
  def __init__(self, bin_dir: str, seq_len: int = 2048):
    ...
    if not bin_files:
      raise FileNotFoundError(
          f"❌ No .bin files found in '{bin_dir}'. Run tokenize_to_bin.py first!"
      )
    ...
    self.total_samples = (
        int(self.cum_samples[-1]) if len(self.cum_samples) > 0 else 0
    )

    local_rank = int(os.environ.get("LOCAL_RANK", 0))
    if local_rank == 0:
      logger.info(
          f"Loaded {len(self.memmaps)} binary shard(s) from '{bin_dir}'"
      )
      logger.info(
          f"   └─ Total Tokens: {total_tokens:,} | Total Sequences"
          f" (L={seq_len}): {self.total_samples:,}"
      )
  ...
  def __getitem__(self, idx):
    ...
    sample_in_file = (
        idx if file_idx == 0 else idx - self.cum_samples[file_idx - 1]
    )
...
def prepare_dataset(args):
  """Instantiates memory-mapped datasets directly from pre-tokenized binary

  directories. Optionally caps validation set to args.max_val_samples if
  configured.
  """
  ...
  if args.max_val_samples is not None and args.max_val_samples > 0:
    if len(val_dataset) > args.max_val_samples:
      ...
      if local_rank == 0:
        ...
        logger.info(
            f"✂️ Capped validation dataset to {args.max_val_samples:,}"
            f" sequences (~{capped_tokens:,} tokens)"
        )
...
def main():
  ...
  parser = argparse.ArgumentParser(
      description="BareTorch High-Throughput Cluster Pre-training Engine"
  )
  ...
  parser.add_argument(
      "--output_dir", type=str, default="./checkpoints_500m"
  )

  # Tokenizer & Dataset Paths
  parser.add_argument(
      "--tokenizer_name",
      type=str,
  ...
  parser.add_argument(
      ...
      help="Dataset name for metadata reference.",
  )
  parser.add_argument(
      "--data_cache_dir",
      type=str,
      default="./tokenized_bin",
      help="Directory containing pre-tokenized uint16 .bin files.",
  )
  parser.add_argument(
      "--max_val_samples",
      type=int,
      default=None,
      help=(
          "Maximum validation sequence samples (L=seq_len). If unset, uses full"
          " validation dataset."
      ),
  )
  ...
  if local_rank == 0:
    logger.info(
        f"Initializing BareTorch Engine. Selected Architecture:"
        f" {args.model_type}"
    )
  ...
  if local_rank == 0:
    logger.info(
        f"Loaded Tokenizer: '{args.tokenizer_name}' | Vocab Size:"
        f" {vocab_size:,}"
    )
  ...
  if args.model_type == "baretorch":
    ...
    if local_rank == 0:
      logger.info(
          f"Assembled Hybrid Sequence ({args.num_layers} layers):"
          f" {layer_types}"
      )
  ...
  if local_rank == 0:
    logger.info(
        "Model initialized successfully. Total parameters:"
  ...
  if os.path.exists(training_args.output_dir):
    ...
    if existing_checkpoints:
      ...
      if local_rank == 0:
        logger.info(
            f"Found existing checkpoint in '{training_args.output_dir}'."
            " Resuming training..."
        )
