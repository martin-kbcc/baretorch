...
def compute_per_token_log_probs(model, input_ids, prompt_len, pad_token_id):
  ...
  with torch.amp.autocast("cuda", dtype=torch.bfloat16):
    ...
    mask[:, prompt_len - 1 :] = (
        shift_labels[:, prompt_len - 1 :] != pad_token_id
    )
...
def main():
  ...
  parser.add_argument(
      ...
      default=None,
      help=(
          "Path to specific GRPO checkpoint directory to resume state from."
      ),
  )
  parser.add_argument(
      "--output_dir",
  ...
  if is_main:
    logger.info(
        f"Initializing Tokenizer '{args.tokenizer_name}' with LEFT padding"
        " and special tokens..."
    )
  ...
  actor_model = BareTorchForCausalLM.from_pretrained(actor_ckpt_path).to(
      local_rank
  )
  ...
  ref_model = BareTorchForCausalLM.from_pretrained(args.checkpoint_dir).to(
      local_rank
  )
  ...
  if is_main:
    logger.info(
        f"Dataset ready: {len(train_dataset):,} train prompts |"
        f" {len(val_dataset):,} val prompts."
    )
  ...
  if args.resume_from_checkpoint and os.path.isdir(args.resume_from_checkpoint):
    ...
    if os.path.exists(state_path):
      ...
      if is_main:
        logger.info(
            f"🔄 Resuming execution from Optimizer Step {start_step}/{total_steps}."
        )
  ...
  if is_main:
    logger.info(
        "🔥 Starting Stage 3: Rule-Based RL (GRPO) Reasoning Alignment..."
    )
    logger.info(
        f"• Dataloader Batches: {len(train_loader)} | Grad Accum:"
        f" {args.grad_accum} | Total Optimizer Steps: {total_steps} | Group Size"
    ...
    for step, batch in enumerate(pbar):
      ...
      if batch_counter <= total_batches_to_skip:
        if is_main and batch_counter % 20 == 0:
          pbar.set_postfix({
              "status": (
                  f"Fast-forwarding batch {batch_counter}/{total_batches_to_skip}"
              )
          })
        continue
      ...
      advantages = ((reward_tensor - mean_r) / (std_r + 1e-8)).view(
          -1
      )  # [B*G]
      ...
      actor_lp, mask = compute_per_token_log_probs(
          actor_model, padded_rollouts, P, tokenizer.pad_token_id
      )

      # Forward pass Reference model (no grad)
      with torch.no_grad():
        ref_lp, _ = compute_per_token_log_probs(
            ref_model, padded_rollouts, P, tokenizer.pad_token_id
        )
      ...
      kl_penalty = (
          torch.exp(ref_lp - actor_lp) - (ref_lp - actor_lp) - 1.0
      )  # Schulman KL estimate

      grpo_loss = ((policy_loss + args.beta * kl_penalty) * mask).sum(
          dim=-1
