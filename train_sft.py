...
def main():
  ...
  parser.add_argument(
      ...
      default="all",
      help=(
          "Dataset subset/config name (e.g., 'all' or"
  ...
  parser.add_argument(
      ...
      default=100000,
      help=(
          "Sub-sample N rows for ultra-dense fast SFT. Set to 0 for full"
          " dataset."
      ),
  )
  ...
  parser.add_argument(
      "--batch_size", type=int, default=12, help="Per-GPU batch size."
  )
  parser.add_argument(
      "--grad_accum", type=int, default=3, help="Gradient accumulation steps."
  )
  parser.add_argument(
      "--learning_rate", type=float, default=5e-5, help="SFT learning rate."
  )
  ...
  if local_rank == 0:
    logger.info(
        "Formatting multi-turn conversations and building assistant loss"
        " masks..."
    )

  processed_dataset = raw_dataset.map(
      lambda example: preprocess_chatml_example(
          example, tokenizer, max_seq_len=args.seq_len
      ),
      batched=False,
