...
PRETRAINED_CHECKPOINT="./checkpoints_500m_hybrid_baretorch/checkpoint-100000"
OUTPUT_DIR="./checkpoints_500m_sft"

# Cloudflare R2 Configurations
R2_BUCKET="baretorch-data"
R2_PREFIX="checkpoints"
R2_REMOTE_PATH="r2:${R2_BUCKET}/${R2_PREFIX}/checkpoints_500m_hybrid_baretorch/checkpoint-100000"

# Hyperparameters
DATASET_NAME="HuggingFaceTB/smoltalk"
DATASET_CONFIG="all"
MAX_SAMPLES=0             # 0 = Use full dataset (~1M samples)
BATCH_SIZE=16             # Per-GPU batch size (80GB VRAM headroom)
GRAD_ACCUM=1              # Global batch size = 16 * 1 * 8 GPUs = 128 sequences
LEARNING_RATE=3e-5        # Optimal SFT learning rate for 500M parameter models
NUM_EPOCHS=2              # 2 passes over instruction dataset
SEQ_LEN=4096              # Matched to native 4K pre-training context window
...
echo "  └─ Global Batch Size      : $(( BATCH_SIZE * GRAD_ACCUM * 8 )) seqs/step ($(( BATCH_SIZE * GRAD_ACCUM * 8 * SEQ_LEN )) tokens/step)"
echo "======================================================================"
...
# Execute DDP via torchrun across 8x H100 GPUs
